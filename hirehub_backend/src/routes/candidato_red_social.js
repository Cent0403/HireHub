...
router.get('/', async (req, res) => {
  try {
    const r = await db.query('SELECT * FROM candidato_red_social');
    res.json(r.rows);
  } catch (err) {
    console.error(err);
...
router.get('/:id', async (req, res) => {
  try {
    const r = await db.query('SELECT * FROM candidato_red_social WHERE id_red_social=$1', [req.params.id]);
...
router.put('/:id', async (req, res) => {
  try {
    ...
    const r = await db.query('UPDATE candidato_red_social SET nombre_red=$1,link=$2 WHERE id_red_social=$3 RETURNING *', [nombre_red, link, req.params.id]);
    if (r.rowCount === 0) return res.status(404).json({ error: 'Not found' });
    res.json(r.rows[0]);
  } catch (err) {
    console.error(err);
...
router.delete('/:id', async (req, res) => {
  try {
    await db.query('DELETE FROM candidato_red_social WHERE id_red_social=$1', [req.params.id]);
    res.status(204).send();
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: err.message });
